...
- **股票隐藏**：可将暂时不想查看的股票设为隐藏，主界面默认不显示也不刷新隐藏股票，勾选“显示隐藏股票”后可管理并恢复显示。
...
- **提醒记录**：触发过的提醒会保存在本地，可通过“提醒记录”入口查看和清空。
...
选中一只股票后点击“隐藏选中”，该股票会保留在本地配置中，但默认从主表隐藏，并暂停该股票的行情刷新和提醒检查。勾选“显示隐藏股票”可以查看隐藏项，选中后点击“恢复显示”即可重新加入默认监控列表。

## 提醒记录

点击主界面的“提醒记录”可以查看历史触发记录，包括触发时间、股票、提醒类型、阈值、触发价格、涨跌幅、是否邮件提醒和消息内容。记录保存在本地 `stocks_config.json` 中，不会上传到 GitHub；最多保留最近 1000 条。
